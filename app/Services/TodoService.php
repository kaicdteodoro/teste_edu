...
class TodoService
{
    ...
    {
        if (!isset($attributtes["title"])) {
            return [
                'success' => false,
                'message' => 'Proibido criar TODO sem um título'
            ];
        }
        DB::beginTransaction();
        try {
     ...
     * @return array
     */
    public function update(array $attributtes, int $todoId): array
    {
        DB::beginTransaction();
        try {
            $todo = $this->repository->update($attributtes, $todoId);
        } catch (\Throwable $th) {
            DB::rollback();
            logger()->error($th);
            return [
